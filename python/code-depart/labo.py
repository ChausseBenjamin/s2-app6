...
def exampleRacines():
    ...
    (z2, p2, k2) = signal.tf2zpk(
        b1, a1
    )  # passage à la représentation zok (liste de racines + "gain")
...
def exampleBode():
    ...
    w1, mag1, phlin1 = signal.bode(
        tf1, np.logspace(-1.5, 1, 200)
    )  # on pourrait aussi laisser la fonction générer les w
...
def exampleButterworth():
    ...
    print(
        f"Butterworth Numérateur {b1}, Dénominateur {a1}"
    )  # affiche les coefficients correspondants au filtre
    print(
        f"Racine butterworth Zéros:{np.roots(b1)}, Pôles:{np.roots(a1)}"
    )  # affichage du resultat dans la console texte
    ...
    u1 = (
        np.cos(2 * np.pi * 0.4 * t1)
        + 0.6 * np.cos(2 * np.pi * 4 * t1)
    ...
    hp.timeplt1(
        np.arange(0, t3m, stept3),
        np.ones_like(tout3),
        tout3,
        yout3,
        "échelon avec signal.step",
    )
...
def probleme2():
    ...
    fsquare = 1  # Hz
    t, step = np.linspace(0, 0.01, 5000, retstep=True)
