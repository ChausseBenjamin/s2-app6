...
\title{S2-APP6: Analyse d'un égaliseur analogique}
...
\section{Frames non-triées TODO: à trier}
...
\begin{frame}
  ...
  \begin{align}
    ...
    \frac{V_e}{R} &= V_x \left( \frac{R}{R} + \frac{1}{Z_{C_2}} + \frac{1}{R_2} \right) - \frac{V_s}{R} \\
    V_e + V_s &= R V_x \left( \frac{R}{R} + \frac{1}{Z_{C_2}} + \frac{1}{R_2} \right) \\
  \end{align}
\end{frame}
...
\begin{frame}
  \frametitle{Papier 4}
  \begin{align}
    V_e &= RV_x \left( \frac{R}{R} + \frac{1}{R_2} + \right)
  \end{align}
\end{frame}
